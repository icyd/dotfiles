...
install_nvr() {
	...
	if [ ! -x "$NVR_BIN" ]; then
		echo -e "${yellow}Creating venv for neovim-remote with python: $PYTHON${reset}"
		$PYTHON -m venv "$PY_VENV/nvr" >/dev/null
		source "$NVR_BIN/activate"
		pip install neovim-remote >/dev/null
		deactivate
		echo -e "${yellow}Creating link for $NVR_BIN/nvr in $TARGET_SYM${reset}"
		ln -sf "$NVR_BIN/nvr" $TARGET_SYM
	else
		echo -e "${green}Neovim remote already installed.${reset}"
	fi
