...
def get_weather_by_ip [locIdx: int, units: string, token: string] {
    ...
    if $units == "f" {
        let units = "imperial"
        let url = $"($URL_WEATHER)?lat=41.390205&lon=2.154007&units=($units)&appid=($token)"
        let url_forecast = $"($URL_FORECAST)?lat=41.390205&lon=2.154007&units=($units)&appid=($token)"
